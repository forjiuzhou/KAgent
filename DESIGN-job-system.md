...
- 批量导入 20 篇论文（sources → canonical/note/synthesis 页 → hub → 交叉链接 → index 更新）
- 大规模整理杂乱 vault（frontmatter 补全、分类调整、孤儿页处理、hub 重组、tag 清理）
- 针对某个领域的 deep research（fetch → 综合 → 产出 synthesis 页 → 更新相关 canonical）
- 重构 vault 结构（调整目录布局、拆分过大的页面、合并重复内容、更新 schema）
...
```markdown
## Acceptance Criteria
- [ ] 每篇论文有对应 wiki 页面 [audit: page count matches source count]
- [ ] frontmatter 完整 [audit: missing_frontmatter = 0]
- [ ] canonical 页面有 sources 字段 [audit: canonical_source_ratio]
- [ ] 每页至少 2 个 wiki-link [audit: avg_links_per_page >= 2]
- [ ] 无孤儿页 [audit: orphan_pages = 0]
- [ ] 页面内容准确，类型选择合理（canonical/note/synthesis） [worker]
- [ ] hub 结构清晰，跨主题关联已体现 [worker]
```
...
## Goal
为 sources/ 下 20 篇论文整理进知识库。根据内容性质选择合适的页面类型
（canonical/note/synthesis），按主题建立或更新 hub，交叉链接，更新 index。

## Acceptance Criteria
- [ ] 每篇论文有对应的 wiki 页面（类型由内容决定） [audit: new page count matches source count]
- [ ] 每个页面 frontmatter 完整（title, type, tags, summary） [audit: missing_frontmatter = 0]
- [ ] canonical 页面有 sources 字段 [audit: canonical_source_ratio]
- [ ] 每页至少 2 个有意义的 [[wiki-link]] [audit: avg_links_per_page >= 2]
- [ ] 无孤儿页 [audit: orphan_pages = 0]
- [ ] 相关主题的 hub 已创建或更新 [audit: hub_coverage]
- [ ] wiki/index.md 反映最新 hub 结构 [worker]
- [ ] 页面内容准确反映原文，类型选择合理 [worker]
- [ ] 跨主题的关联通过 synthesis 页或 wiki-link 体现 [worker]
...
- 创建 canonical: wiki/concepts/attention-mechanism.md（有明确原始来源）
- 创建 note: wiki/concepts/multi-modal-learning.md（初步整理，待后续补充）
- 创建 synthesis: wiki/synthesis/attention-vs-convolution.md（跨论文比较）
- 创建 hub: wiki/concepts/deep-learning-hub.md
- 更新 wiki/index.md 添加 Deep Learning hub

### 文件变更说明
- wiki/concepts/attention-mechanism.md: 新建 canonical，基于 paper-a.pdf，覆盖 self-attention 和 multi-head attention，sources 指向原文
- wiki/concepts/multi-modal-learning.md: 新建 note，paper-c.pdf 内容较杂，先以 note 形式整理
- wiki/synthesis/attention-vs-convolution.md: 新建 synthesis，对比 paper-a 和 paper-d 的方法论差异
...
### 自评
前 5 篇处理顺利。paper-c 内容跨多个主题，先以 note 收录，
后续积累更多素材后可能升级为 canonical 或拆分。
分类暂时全放 Deep Learning 下，后续如果出现 NLP 或 CV 相关论文
可能需要拆分 hub。
...
Agent: 我看了 sources/，有 20 个文件。我建议创建一个后台任务：
  ...
  目标：为每篇论文整理进知识库，根据内容选择合适的页面类型，
        按主题组织 hub，建立交叉链接
  验收标准：
    1. 每篇论文有对应 wiki 页面（canonical/note/synthesis） [audit]
    2. 每个页面 frontmatter 完整 [audit]
    3. canonical 页面有 sources 字段 [audit]
    4. 每页至少 2 个有意义的 wiki-link [audit]
    5. 无孤儿页 [audit]
    6. 相关 hub 已创建或更新 [audit]
    7. 页面内容准确，类型选择合理 [worker]
    8. 跨主题关联已体现 [worker]
  最多 30 轮
...
Agent:（从 system prompt 注入看到 job 状态）
      ...
      - 创建了 14 个 canonical 页、4 个 note、2 个 synthesis
      - 建立了 4 个主题 hub
      - 交叉链接 47 个
      所有验收标准满足。你可以查看进度详情。
```
