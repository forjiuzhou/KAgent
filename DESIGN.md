...
```
[Knowledge Operations — 设计层面的能力类别]
...
> **实现现状**：上述 9 个类别是设计层面的分类。在代码实现中，这些类别被细化为 **18 个具体工具**（如 `read_page`, `write_page`, `append_section`, `search_vault`, `fetch_url` 等），详见 `src/noteweaver/tools/definitions.py`。
...
- **Syncthing（已采纳）**：外部工具实时双向同步 vault 目录，无需代码集成，见 [deploy/README.md](./deploy/README.md)。

> **当前实现**：同步不内建在代码中。部署文档推荐使用 **Syncthing** 在 VPS 网关与本地电脑之间实时同步 vault 目录。Git 自动 commit 由 Vault 层自动完成，但不自动 push 到远端。
...
| **可执行操作** | 几乎无限 | 几乎无限（受限） | 18 种知识操作工具（9 类原语） |
| **最坏情况** | 系统被完全控制 | 文件被误删改 | 知识库笔记被错误修改（可回滚） |
| **目标用户** | 开发者 | 开发者 | 所有人 |

关键安全设计：

1. **操作即白名单**：Agent 只能执行预定义的 18 种知识操作工具（归属 9 类原语），不存在"逃逸"到任意执行的路径
...
- Tool 分组的概念（我们的 18 种操作工具就是一个极简的 tool 集合）
...
> **注意**：§7.1–§7.9 是早期设计阶段的选型讨论，当时考虑了 TypeScript/Bun 方案。经过讨论后最终选择了 **Python**（见 §7.10）。保留这些讨论作为决策上下文参考，**实际实现使用的是 §7.10 中的 Python 技术栈**。
...
**已完成**：
- ✅ MVP v1：CLI 交互对话、Web 导入、知识库健康检查、自动 Git commit、rebuild-index
- ✅ MVP v2：Telegram Bot 网关（`nw gateway`）、定时 digest/lint、通知队列
- ✅ Journal 系统：会话自动沉淀、digest 提取洞见、promote_insight 晋升到 wiki
- ✅ 双 Provider 支持：OpenAI + Anthropic
- ✅ 18 种知识操作工具、frontmatter 硬约束校验、归档替代删除
- ✅ Syncthing 部署方案文档、Docker/systemd 部署

**待做**：
1. 执行成功标准测试：10 篇文章 → 编译 wiki → 综合查询 → 对比 RAG
2. Feishu 适配器实现（`lark-oapi` 已作为可选依赖，适配器待开发）
3. 交互式 HTML/JS 可视化输出（Fridman 模式 / RENDER 操作）
4. 迷你知识库投射 + 语音模式
5. Web UI、桌面应用
6. 本地模型支持（Ollama）
7. 更多平台入口（微信、Discord 等）
